...
export class JmapClient {
  ...
  async listMessages({
    ...
    const { methodResponses } = json;
    const emailQueryResponse = methodResponses?.find((r) => r[0] === "Email/query");
    ...
    // Extract pagination info from the query response
    const queryResult = emailQueryResponse ? emailQueryResponse[1] : {};
    const ids = queryResult.ids || [];
    const queryPosition = queryResult.position ?? 0;
    const hasMore = ids.length >= limit;

    return {
      messages: emailGetResponse[1].list,
  ...
  async searchMessages({
    ...
    // Build filter conditions from individual parameters
    const conditions = [];
    if (explicitFilter) {
      conditions.push(explicitFilter);
    }
    if (from) conditions.push({ from });
    if (to) conditions.push({ to });
    if (replyTo) conditions.push({ replyTo });
    if (cc) conditions.push({ cc });
    if (bcc) conditions.push({ bcc });
    if (subject) conditions.push({ subject });
    if (body) conditions.push({ body });
    if (query) conditions.push({ text: query });
    if (after) conditions.push({ after: after });
    ...
    const ids = queryResult.ids || [];
    const queryPosition = queryResult.position ?? 0;
    const hasMore = ids.length >= (limit || ids.length);
