...
// dotenv is a Node.js module used only when a config file path is explicitly
// provided.  We load it lazily so that browser bundlers don't fail on the
// static import of a Node-only package.
let _dotenv;
try {
  _dotenv = (await import("dotenv")).default;
} catch {
  // dotenv not available (e.g. browser) — .env file loading will be skipped
}
...
export class JmapClient {
  ...
  constructor(options = {}) {
    ...
    if (path) {
      ...
      // In environments where dotenv is unavailable (e.g. browser),
      // the .env file is simply skipped.
    }
  ...
  async sendEmail({ from, fromName, to, subject, text, attachment, replyTo, replyMessageId }) {
    ...
    if (replyMessageId) {
      ...
      email.headers = [
        { name: "In-Reply-To", value: replyMessageId },
      ];
    }
    if (attachment) {
