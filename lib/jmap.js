...
// ---------------------------------------------------------------------------
// Supports both Basic Auth (default) and OAuth2 authentication.
// OAuth2 mode handles auto-discovery, password grant, refresh tokens,
// automatic expiry refresh, and single 401 retry.
// ---------------------------------------------------------------------------
...
export class JmapClient {
   ...
   * @param {string}  [options.username]        JMAP username (also: JMAP_USERNAME env)
   * @param {string}  [options.password]        JMAP password (also: JMAP_PASSWORD env)
   * @param {string}  [options.baseUrl]         JMAP base URL (also: JMAP_BASE_URL env)
   * @param {string}  [options.path]            Path to dotenv config file
   *
   * // --- OAuth2 options ---
   * @param {string}  [options.authType]        "basic" (default) | "oauth2" (also: JMAP_AUTH_TYPE env)
   * @param {string}  [options.accessToken]     Pre-existing OAuth2 access token (also: JMAP_ACCESS_TOKEN env)
   * @param {string}  [options.refreshToken]    Pre-existing OAuth2 refresh token (also: JMAP_REFRESH_TOKEN env)
   * @param {string}  [options.clientId]        OAuth client ID (also: JMAP_CLIENT_ID env, default: "jmap-client")
   * @param {string}  [options.tokenEndpoint]   Explicit token endpoint (also: JMAP_AUTH_TOKEN_ENDPOINT env)
   * @param {boolean} [options.autoRefresh]     Auto-refresh tokens (also: JMAP_AUTO_REFRESH env, default: true)
   */
  constructor(options = {}) {
    let {
      username,
    ...
    if (!authType) authType = process.env.JMAP_AUTH_TYPE || "basic";
    if (!accessToken) accessToken = process.env.JMAP_ACCESS_TOKEN;
    ...
    if (autoRefresh === undefined) {
      const env = process.env.JMAP_AUTO_REFRESH;
      autoRefresh = env !== undefined ? env !== "false" : true;
    }
    ...
    // If Basic Auth is explicitly selected (or default) and no OAuth config is present,
    // compute the Basic auth header right away.
    if (this.authType === "basic") {
      ...
      // OAuth2 mode
      this.apiUrl = `${this.baseUrl}/jmap`;
      this.authHeader = null; // not used in OAuth2 mode
  ...
  /**
   * Make a JMAP API request with automatic auth-header selection,
   * token refresh on 401, and single retry.
   *
  ...
  async _request(url, options = {}) {
    ...
    if (res.status === 401 && this._tokenManager) {
      try {
        ...
        // If refresh failed with revoked error, re-throw it
   ...
   * Same as _request but calls .json() on the response.
   *
   * @param {string} url
   * @param {object} options
  ...
  // -----------------------------------------------------------------------
  // Real-time listen (EventSource / WebSocket)
  // -----------------------------------------------------------------------
  ...
  _getWebSocketUrl(session) {
    ...
    return `${this.baseUrl.replace(/^http:/, "ws:").replace(/^https:/, "wss:")}/jmap/ws`;
  }
