...
export function getBasicAuthHeader() {
  const { JMAP_USERNAME, JMAP_PASSWORD } = process.env;
  if (!JMAP_USERNAME || !JMAP_PASSWORD)
    throw new Error("JMAP_USERNAME and JMAP_PASSWORD must be set in .env");
  const token = Buffer.from(`${JMAP_USERNAME}:${JMAP_PASSWORD}`).toString(
    "base64",
  );
  return `Basic ${token}`;
}
